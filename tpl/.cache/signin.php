    ...
    <body style="background: #f7fafc;vertical-align: middle;margin-top: 55px">
        <div class="wraper center-block" style="width:280px">
            ...
            <form method="post">
                ...
                <input name="token" type="hidden" value="<?php echo htmlspecialchars(empty($_GET['token'])?'':$_GET['token']);?>" />
                <input name="refer" type="hidden" value="<?php echo htmlspecialchars(empty($_GET['refer'])?'':$_GET['refer']);?>" />
            </form>
        </div>
        <script>
            $(function(){
